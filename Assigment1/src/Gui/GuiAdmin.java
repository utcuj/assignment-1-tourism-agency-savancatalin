...
public class GuiAdmin {
	...
	public void afiseazaChangeInTabelDate() throws SQLException  {
		    changeGrafic.setRowCount(0);
		    SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss");
		    Timestamp ts =Timestamp.valueOf(data1.getText());
		    Timestamp ts1=Timestamp.valueOf(data2.getText());
		    
		    System.out.println(data2.getText());
		    
		    IstoricService data= new IstoricService();
			Istoric change = data.findByData(ts, ts1);
			String[] OB = {Integer.toString(((Istoric) change).getIdUser()),					
					(((Istoric) change).getChange()),dateFormat.format(((Istoric) change).getTimp())};
